  ...
  const handleApply = (job: Job) => {
    ...
    setTimeout(() => {
      if (roll <= chance) {
        dispatch({
          type: 'SET_JOB',
          payload: { title: job.title, salary: job.salary, tier: job.tier }
        });
      }
      setApplyingFor(null);
    }, 500);
        ...
        <Card className="bg-zinc-900 border-zinc-700">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="pt-4 border-t border-zinc-700">
              <Button variant="destructive" onClick={handleQuit} className="w-full">
                Quit Job
              </Button>
            </div>
          </CardContent>
      ...
      <Card className="bg-zinc-900 border-zinc-700">
        ...
        <CardContent className="space-y-3">
          {availableJobs.map((job) => {
            ...
            return (
              <div key={job.id} className="p-3 bg-zinc-800 rounded-lg border border-zinc-700 space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-medium text-zinc-100">{job.title}</div>
                    <div className="text-xs text-zinc-400">{job.description}</div>
                  </div>
                  <Badge className={`${getTierColor(job.tier)} text-xs`}>{job.tier}</Badge>
                </div>
                <div className="flex items-center gap-4 text-sm">
                  ...
                  <div className="text-zinc-400">Success: {Math.round(chance)}%</div>
                </div>
                {job.requirements.intelligence && (
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className={character.stats.intelligence >= job.requirements.intelligence ? 'text-green-400' : 'text-red-400'}>
                      INT {job.requirements.intelligence}+ ({character.stats.intelligence >= job.requirements.intelligence ? '✓' : '✗'})
                    </span>
                  </div>
                )}
                <Button 
